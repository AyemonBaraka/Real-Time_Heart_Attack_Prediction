...
# Step 1: Apply preprocessing to inspect the features column
preprocessing_pipeline = Pipeline(stages=[assembler1, scaler, ohe, assembler2])
preprocessed_model = preprocessing_pipeline.fit(trainDF)
preprocessed_trainDF = preprocessed_model.transform(trainDF)

# Inspect the features column
print("Inspecting the features column:")
preprocessed_trainDF.select("features").show(1, truncate=False)

# Step 2: Define the MLP model
# Adjust the layers based on the number of input features
# Example: Input features = 30 (adjust based on inspection), hidden layers = [15, 10], output = 2 (binary classification)
layers = [22, 15, 10, 2]

mlp = MultilayerPerceptronClassifier(labelCol="label", featuresCol="features", layers=layers, seed=42, maxIter=100)

# Step 3: Create the full pipeline
mlp_pipeline = Pipeline(stages=[assembler1, scaler, ohe, assembler2, mlp])

# Train the model
print("Training the MLP model...")
pModel = mlp_pipeline.fit(trainDF)
